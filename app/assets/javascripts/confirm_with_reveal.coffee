# https://github.com/ubermanl/confirm-with-reveal
...
$.fn.extend
	confirmWithReveal: (options = {}) ->
		...
		do_confirm = ($el) ->
			...
			if (typeof el_options is 'string') and (el_options.length > 0)
				return ($.rails?.confirm or window.confirm).call(window, el_options)

			option = (name) ->
				el_options[name] or settings[name]
			...
			confirm_button
				...
				.on 'click', (e) ->
					...
					confirm_button.text(option 'confirm_applied')
					confirm_button.attr('disabled', true)
					modal.find('[data-confirm-cancel]').hide()
					$el.trigger('reveal', e)
					if $el.is('form, :input')
						$el
							.closest('form')
			...
			if (password = option 'password')
				...
				modal
					...
					.on 'keyup', (e) ->
						disabled = $(this).val() isnt password
						confirm_button
